...
// Database Seeder (Seeds professional initial records if database is fresh)
export async function seedInitialDataIfEmpty() {
  try {
    const lawyersSnap = await getDocs(collection(db, FIRESTORE_COLLECTIONS.LAWYERS));
    if (lawyersSnap.empty) {
      console.log("Seeding initial professional law firm data into Firestore...");
      
      // Seed default lawyers
      const defaultLawyers = [
        {
          userId: "lawyer_sarah_jenkins",
          displayName: "Sarah Jenkins, Esq.",
          email: "sarah.jenkins@lexcounsel.example",
          specialization: "Corporate & Business Litigation",
          registrationNumber: "BAR-NY-849201",
          bio: "Senior Partner specializing in commercial transactions, M&A due diligence, and contract disputes with 14+ years experience.",
          availability: "Mon, Wed, Thu (9 AM - 4 PM)",
          active: true,
          createdAt: serverTimestamp()
        },
        {
          userId: "lawyer_marcus_vance",
          displayName: "Marcus Vance, J.D.",
          email: "marcus.vance@lexcounsel.example",
          specialization: "Real Estate & Landlord-Tenant",
          registrationNumber: "BAR-NY-918234",
          bio: "Lead real estate counsel representing commercial landlords, purchasers, and property developers.",
          availability: "Tue, Thu, Fri (10 AM - 5 PM)",
          active: true,
          createdAt: serverTimestamp()
        },
        {
          userId: "lawyer_elena_rostova",
          displayName: "Elena Rostova, LL.M.",
          email: "elena.rostova@lexcounsel.example",
          specialization: "Family & Estate Law",
          registrationNumber: "BAR-NY-726481",
          bio: "Experienced family law attorney handling divorce mediation, trust agreements, and probate administration.",
          availability: "Mon - Fri (8:30 AM - 3 PM)",
          active: true,
          createdAt: serverTimestamp()
        }
      ];
      ...
      for (const l of defaultLawyers) {
        await setDoc(doc(db, FIRESTORE_COLLECTIONS.LAWYERS, l.userId), l);
        await setDoc(doc(db, FIRESTORE_COLLECTIONS.USERS, l.userId), {
      ...
      // Seed default firm settings
      await setDoc(doc(db, FIRESTORE_COLLECTIONS.SETTINGS, "firmConfig"), {
        firmName: "LexCounsel Legal Services LLP",
        email: "contact@lexcounsel.example",
        phone: "+1 (800) 555-0199",
        address: "500 Chancery Row, Suite 400, New York, NY 10005",
        businessHours: "Monday – Friday: 8:30 AM – 6:00 PM EST",
        emergencyContact: "+1 (800) 555-0190",
        updatedAt: serverTimestamp()
      });
    }
  } catch (err) {
